...
class UDFResolverSuite extends AnyFunSuite with BeforeAndAfterEach {
  ...
  test("a name colliding with a spark built-in throws") {
    UDFResolver.UDFNames += "abs"
    val e = intercept[GlutenException](describedNames())
    assert(e.getMessage.contains("UDF 'abs'"))
    assert(e.getMessage.contains("shadows a Spark built-in"))
  }

  test("every name colliding with a spark built-in is reported") {
    UDFResolver.UDFNames ++= Seq("abs", "myudf_increment", "upper")
    val e = intercept[GlutenException](describedNames())
    assert(e.getMessage.contains("UDF 'abs', 'upper'"))
    assert(e.getMessage.contains("shadow Spark built-ins"))
    assert(!e.getMessage.contains("myudf_increment"))
  }
