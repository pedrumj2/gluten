...
This is off by default. Set `spark.gluten.sql.columnar.backend.velox.nativeUDFWithoutFallback=true` to turn it on.
...
| spark.gluten.sql.columnar.backend.velox.nativeUDFWithoutFallback | Call a UDF by the name it was registered with, without writing a Java class for it. There is then no Java version to fall back to, so a query Gluten cannot run natively will fail. Defaults to `false`. |
